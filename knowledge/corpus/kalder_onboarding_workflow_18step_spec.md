...
**Input required:** (a) Confirm which Outreach sequence IDs are configured for each active convergence point. (b) Confirm channel-variant sequences are configured for `msp` and `partner` accounts if applicable. (c) Confirm two contact-level gates are implemented: Gate 1 (`confidence_tier: MEDIUM` or higher) and Gate 2 (`differential_insufficient: false`). *Note: `confidence_tier` is the role classification confidence measure — HIGH, MEDIUM, LOW, or UNKNOWN — that governs which experience level the personalization engine delivers to each visitor. It is the primary entity you will see in every Advisor output. The closing orientation below maps how the attributes you configured in this onboarding session feed into `confidence_tier` and the other working entities.*  
...
## What You Just Configured — and Where You'll See It

You have completed the 18-step onboarding sequence. Every attribute you configured in this session feeds into a decisioning entity that the Advisor references in every output — in Reference Mode responses, in Advisory Mode diagnoses, and in the Guided Workflow steps you will use when operating the program day-to-day. This orientation names those entities, maps them to what you configured, and closes the vocabulary gap between the configuration dialect you just worked in and the working dialect you are about to use.

---

### The Two Core Measures

Every Advisor output centers on two measures. They use different numeric ranges, answer different questions, and must not be substituted for each other.

**Role Confidence** (`confidence_tier`) — *the measure that governs experience depth.*

Role Confidence answers the question: how precisely has this visitor been classified? The scoring pipeline produces a numeric score for each contact in each solution category context. That score resolves to one of four tiers:

| Tier | Score range | What it means | Experience delivered |
|---|---|---|---|
| `HIGH` | ≥ 80 | Classification is certain — behavioral and zero-party signals agree | Level 1: Role-specific experience |
| `MEDIUM` | 50–79 | Classification is probable — behavioral signals strong but not confirmed | Level 2: Role-influenced experience |
| `LOW` | 25–49 | Classification is uncertain — limited signal | Level 3: Solution-interest experience |
| `UNKNOWN` | < 25 | Classification is not possible at this time | Level 4 or Level 5: Account-level or default |

Role Confidence is a buyer-centric measure. Its owner is Analytics and Content Operations. When you see `confidence_tier` in an Advisor output, it is telling you how much the program knows about a specific contact at this moment.

**Engagement Score** — *the measure that governs sales activation intensity.*

Engagement Score answers the question: how urgently should we pursue this account? It aggregates contact-level engagement intensity across identified buying group members to produce an account-level deal-health signal. Tiers are `LOW_ENGAGEMENT` (0–39), `MEDIUM_ENGAGEMENT` (40–69), `HIGH_ENGAGEMENT` (70–100). This is a seller-centric measure. Its owner is Sales and Revenue Operations.

**The collision warning.** Both measures use a 0–100 numeric scale. A score of 72 is `HIGH_ENGAGEMENT` but `MEDIUM` confidence. A score of 45 is `LOW` confidence but `MEDIUM_ENGAGEMENT`. These are measurements of different things on the same scale. Do not use one as a proxy for the other.

---

### The Five Experience Levels

Role Confidence drives the fallback cascade — the five levels of personalization depth the experience engine can deliver. Level is selected at session start for each visitor and held stable for that session.

| Level | Name | Trigger condition | What the visitor sees |
|---|---|---|---|
| **Level 1** | Role-specific experience | `confidence_tier: HIGH` | Content, CTAs, and framing fully tailored to the classified role and solution category. Three-axis personalization activates if buying job confidence is KNOWN or INFERRED. |
| **Level 2** | Role-influenced experience | `confidence_tier: MEDIUM` | Content leans toward the classified role but retains safe fallback elements. No highly role-specific CTAs. |
| **Level 3** | Solution-interest experience | `confidence_tier: LOW` or `UNKNOWN` with identified solution interest | Content organized by solution category without role-specific framing. No role assumptions. |
| **Level 4** | Account-level experience | TAL account identified — no solution interest or role signal | Personalized by industry, company size, or account attributes only. |
| **Level 5** | Default brand experience | Non-TAL visitor, holdback assignment, or suppressed consent state | Standard brand experience. No personalization. |

`fallback_level` is the numeric form of this cascade (1 through 5). When the Advisor references a contact's `fallback_level`, it is specifying which row of this table applies to that contact at this session.

---

### What You Configured — and Where It Appears

| What you configured | Attribute | What it feeds | Where you'll see it |
|---|---|---|---|
| Account domain for reverse-IP matching | `tal_domain` | Demandbase Layer 1 identification → `confidence_tier` and `fallback_level` for anonymous TAL visitors | Advisor outputs describing anonymous visitor experience; Level 4 and above activation status |
| TAL membership flag | `tal_member` | Primary program gate — all scoring, classification, and experience selection depends on this being `true` | Every Advisor output about this account; Level 5 is the ceiling for `tal_member: false` |
| Account type and program status | `tal_account_type_source`, `tal_program_status` | Cohort assignment (`bg_cohort`) → determines which content treatment path activates | Advisor cohort diagnostics; acquisition vs. upsell treatment routing |
| Region | `tal_region` | Regional campaign assignment and geographic consent rule triggering | Advisor outputs flagging consent-state behavior for EMEA/APAC accounts |
| Solution interest flags | `tal_solution_interest_flags` | Level 4 solution-specific content selection; input to solution_category classification | Level 3 and Level 4 experience selection when role classification is unavailable |
| New logo eligibility | `tal_new_logo_eligible` | Acquisition personalization gate | Advisor outputs on acquisition cohort activation scope |
| Open pipeline and opportunity stage | `tal_open_pipeline`, `sfdc_opportunity_created`, `sfdc_opportunity_stage` | `progression_win_now` cohort entry and elevated-priority convergence point alert routing | Advisor sales activation diagnostics; convergence point alert severity |
| Consent state configuration | `visitor_consent_state`, Track 2 status, signal classifications | Signal availability for scoring → directly constrains which `confidence_tier` is achievable | Every `confidence_tier` output — consent state is the upstream gate on all signal collection |
| Outreach gate thresholds | Gate 1 (`confidence_tier: MEDIUM`+), Gate 2 (`differential_insufficient: false`) | Sales activation eligibility — which contacts receive Outreach sequence triggers | Advisor sales activation outputs; contact-level gate pass/fail status |
| AEP → Salesforce connector | Connector confirmation (Step 17) | Delivery path for all account-plane attributes and convergence point alerts to the sales team | Advisor integration health outputs; connector status in operational diagnostics |

---

### The Three Advisor Interaction Modes

When you work with the Advisor after onboarding, you will use one of three interaction modes depending on what you need:

**Reference Mode** — Retrieve specific corpus rules, attribute definitions, thresholds, or canonical values. Use when you need a precise answer from the corpus: "What is the `confidence_tier` threshold for Level 1 activation?" or "What does `differential_insufficient: true` mean?" Reference Mode returns corpus-grounded answers with source citations. Available in all tiers.

**Advisory Mode** — Diagnose a program state, interpret a pattern, or reason through a strategic question. Use when you need the Advisor to apply judgment, not just retrieve: "Why are so many contacts sitting at Level 3 for IT Operations?" or "Is this confidence distribution a signal or a signal failure?" Advisory Mode synthesizes corpus knowledge against your program's specific state. Available in Professional and Enterprise tiers.

**Guided Workflow Mode** — Step-by-step execution of a structured process. The 18-step onboarding you just completed was a Guided Workflow. Other workflows include content commissioning review, holdback analysis preparation, and convergence point alert configuration. Guided Workflow Mode enforces sequencing, interrupts on HOLDs and FLAGs, and produces an audit log. Available in Enterprise tier; onboarding workflow available in all tiers.

*For depth on each mode — including persona-specific entry paths, the confidence threshold that separates Reference from Advisory reasoning, and the full HOLD/FLAG interrupt architecture — see the Kalder AI Advisor Conceptual Model.*

---

### Your Configuration State Entering the Working Surface

When you open an Advisor session after onboarding, your configuration is active. The attributes you confirmed in Steps 1–18 are registered in `CLIENT_ATTRIBUTE_MAP` and are what the Advisor reads when it evaluates this account's program state. If you deferred any steps, the Configuration Gap Record lists the consequences — and the Advisor will surface those gaps when they become relevant to an output.

The onboarding session is complete. The Advisor is ready.
